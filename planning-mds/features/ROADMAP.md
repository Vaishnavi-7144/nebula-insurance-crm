...
|---------|-------|---------|
| [F0038 — Neuron Day-at-a-Glance Shell (Renewals live + draft outreach + mock-send)](./F0038-neuron-day-at-a-glance-shell/README.md) | Neuron Companion | New epic (intake brief signed off 2026-06-29). First slice of the Neuron Companion: a Day-at-a-Glance shell with the Renewals zone live + one assisted write (draft outreach) + mock-send — proves the companion chain end-to-end and lays the multi-head pathway. Assembly only (cross-zone brain deferred). **Skeleton — needs its own `plan` run before build; AI Engineer role buildout is a prerequisite.** |
